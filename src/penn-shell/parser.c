...
void print_parsed_command_without_end(struct parsed_command* const cmd) {
  for (size_t i = 0; i < cmd->num_commands; ++i) {
    for (char** arguments = cmd->commands[i]; *arguments != NULL; ++arguments) {
      write(STDERR_FILENO, *arguments, strlen(*arguments));
      write(STDERR_FILENO, " ", 1);
    }

    if (i == 0 && cmd->stdin_file != NULL) {
      write(STDERR_FILENO, "< ", 2);
      write(STDERR_FILENO, cmd->stdin_file, strlen(cmd->stdin_file));
      write(STDERR_FILENO, " ", 1);
    }
    ...
    if (i == cmd->num_commands - 1) {
      if (cmd->stdout_file != NULL) {
        ...
        write(STDERR_FILENO, cmd->stdout_file, strlen(cmd->stdout_file));
        write(STDERR_FILENO, " ", 1);
      }
    } else {
      write(STDERR_FILENO, "| ", 2);
